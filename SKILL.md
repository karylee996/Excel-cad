...
6. Do NOT return the intermediate `__working.dxf`.
7. The safe runner must produce both:
   - the final validated DXF;
   - a `*_validation.json` manifest.
8. The validation manifest must contain:
...
10. Before sharing the final artifact, confirm the final path exists and its bytes correspond to the validated file referenced in the manifest.

The purpose of this contract is to prevent a common failure mode where a newly generated DXF is parsable by a Python library but AutoCAD opens an empty `Drawing1` or asks the user to press Enter before showing a blank drawing.
...
1. Generate a working DXF through the repository safe runner.
...
11. Write the `*_validation.json` manifest.
12. Only after all checks pass may the DXF be returned.

Use `scripts/run_excel_cad_safe.py` as the mandatory entry point. Any dimension-placeholder post-processing must happen before the final AutoCAD compatibility pass.
...
3. **Validation manifest JSON** generated by the safe runner.
4. Preview PNG when rendering is available.

## Final quality checks

Before returning any DXF:
- actual repository safe runner was executed;
- validation manifest exists;
- validation manifest contains `build_marker == EXCEL_CAD_SAFE_BUILD_V2`;
- validation manifest contains `passed == true`;
...
If any compatibility or execution check fails, do **not** return the broken DXF. Repair the repository workflow or rerun it in the same task and validate again.
